...
const ChecklistDetails = ({ route }) => {
  ...
  const getDetail = async () => {
    await axios
      .get(
        `https://tstapp.poscoassan.com.tr:8443/WorkOrder/MMS/GetCareSystemChecklistRecord/${item.uID}`,
        {
          headers: {
      ...
      .then((res) => {
        if (res.data?.status === "Success") {
          ...
          // istediğin format
          const formatted = flatData
            .filter((item) => !isNaN(Number(item.Answer)))
            .map((item) => ({
      ...
      <ScrollView>
        {details.length > 0 ? (
          <LineChart
            ...
            height={height * 0.8}
            width={width * 0.9}
            spacing={50} // x-axis item aralığı
            xAxisLabelTextStyle={{
              fontSize: 10,
            ...
            >
              <Card.Content>
                <Text variant="titleLarge">
                  <Text style={{ fontWeight: "bold" }}>Checlist:</Text>
                  {item.CheckItem}
                </Text>
                <Text variant="bodyMedium">
                  <Text style={{ fontWeight: "bold" }}>Yanıt:</Text>
                  {item.Answer}
                </Text>
                <Text variant="bodyMedium">
                  <Text style={{ fontWeight: "bold" }}>Kontrol Tarihi:</Text>
