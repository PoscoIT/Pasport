...
const newDate = new Date(new Date(date).setMinutes(date.getMinutes() + 15))
    ...
    const getLineList = async () => {
        try{
            await axios.get(`${url}Common/ListSubTeamData`,{
                params:{
    ...
    const handleEmployeeID = async () => {
        ...
        if (employeeID.length !== 6) {
            ...
            await axios.get(`${url}/UserAccount/ListUserMainDepartment?EmployeeID=` + employeeID,{   headers: {
                    'auth-token': REACT_APP_SECRET_KEY,
                },}).then((res)=>{
            ...
            <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
                <View style={{flex: 1}}>
                    ...
                    <TextInput
                        ...
                        label={t("employeeID")}
                    />
                    {name !== '' && employeeID.length===6 && <Text style={{margin:10,fontWeight:"bold"}}>{name}</Text>}
               ...
               <PickerModal
                    ...
                    style={{backgroundColor:"#fff",marginVertical:15,borderColor:"#757575",borderRadius:10}}
...
*/}
                    <View style={{flexDirection:"column",display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:30}}>

                        <Button style={{color:"red"}} color={"black"}  onPress={() => setOpen(true)} >
                            Başlangıç Tarihi
                        </Button>
                        <Text onPress={() => setOpen(true)}>{date.toLocaleString('tr-TR')}</Text>
                        <Button  style={{color:"red"}} color={"black"}   onPress={() => setOpen2(true)} >
                            Bitiş Tarihi
                        </Button>
                        <Text onPress={() => setOpen2(true)}>{date2.toLocaleString('tr-TR')}</Text>
                    </View>
