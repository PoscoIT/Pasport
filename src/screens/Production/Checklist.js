...
const Checklist = ()=>{
    ...
    const getQuestionList = async ()=>{
        await axios.get("https://tstapp.poscoassan.com.tr:8443/WorkOrder/GetChecklistQuestion/10").then((res)=>{
            if(res.data){
    ...
    <ScrollView  style={styles.view}>
                            ...
                            <RadioGroup
                                selectedIndex={
                                    formValues[item.ID] !== undefined
                                        ? answers.findIndex(ans => ans.value === formValues[item.ID])
                                        : null
                                }
                                onChange={index => handleAnswerChange(item.ID, answers[index].value)}
                                style={styles.radio}
                            >
                                {answers.map(ans => (
        ...
        {questionList.length>0?  <Button style={{marginBottom:20,marginHorizontal:50}} onPress={onSubmit}>
            Checklisti Tamamla
        </Button>:
            <View>
