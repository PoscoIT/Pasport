...
import {ActivityIndicator, Button, Portal, Provider, Dialog, DefaultTheme, HelperText} from 'react-native-paper';
...
const MyMachineScreen = ({navigation}) => {
    ...
    const sendData = async()=>{
        ...
        await axios.post(`${url}/WorkOrder/CreateMyMachine`,formdata,{
            headers: {
    ...
    const sendForm = async () => {
            ...
            if(!filePath || !filePath2 || !currentProblem || !selectedCategory || !equipmentText || !finishAction ){
    ...
    const getCategory = async () => {
        try{
            await axios.get(`${url}/WorkOrder/GetMyMachineCategory`,{
                headers:{
                    "auth-token":REACT_APP_SECRET_KEY,
    ...
    const getDepartmentList = async () => {
        try{
            if(employeeId){
                await axios.get(`${url}/UserAccount/GetDepartmentInfo`,{
                    params:{
                        sicilNo:employeeId
                    },
                    headers:{
    ...
    const getMainOperator = async () => {
        try{
            if(employeeId){
                await axios.get(`${url}/UserAccount/ListUserMainOperator`,{
                    params:{
                        sicilNo:employeeId
                    },
                    headers:{
                    ...
                    setMainOperatorList(res?.data?.map(item => [{
                        ...
                        <View style={styles.form}>
                           ...
                           <>
                                ...
                                <View style={{width:"100%"}}>
                                    <Icon.Button name="image" backgroundColor={"#69b5fa"} onPress={() => setVisible2(true)}>
                            ...
                            <View style={{flex: 1, alignItems: "center", marginVertical: 10}}>
                                {loading ? <ActivityIndicator animating={true} color={theme.colors.primary}/> :
                                    <Button style={styles.button} disabled={loading} onPress={sendForm}
